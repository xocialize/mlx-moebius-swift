...
//
// Moebius hits the window in the SDXL KL-f8 VAE (20 encoder convs per encode — two encodes per
// inpaint — and 31 decoder convs at 512²) and in one UNet conv per denoise step (the
// `up_blocks.1` upsampler, 640→640 at 64², batch 2). fp16 weights meet fp32 activations, so these
// convs compute in fp32. Defaults: encoder `.conv3d` (the masked-image latents feed the UNet),
// UNet upsampler `.conv3d` (one conv per step — cheap), decoder `.winograd` (its fp32 loss is below
// 8-bit visibility; parity lanes opt in).
...
// `MOEBIUS_CONV_ROUTE=winograd|conv3d|fp32Winograd` overrides the defaults (validation).
...
public enum MoebiusConvRoute: String, Sendable {
    ...
    /// `MOEBIUS_CONV_ROUTE` = winograd | conv3d | fp32Winograd, if set.
    static var environmentOverride: MoebiusConvRoute? {
        getenv("MOEBIUS_CONV_ROUTE").flatMap { MoebiusConvRoute(rawValue: String(cString: $0)) }
    }
}

final class WinogradFreeConv2d: Conv2d {
    var route: MoebiusConvRoute = .conv3d
